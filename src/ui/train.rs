...
impl TrainSimulation {
    ...
    pub fn draw(&self, origin: Vec2, scale: f32, texture: Option<&Texture2D>) {
        ...
        let train_center = origin + Vec2::new(train_x, loco_y) * scale;
        let render_w = TRAIN_WIDTH * scale;
        let render_h = TRAIN_HEIGHT * scale;

        // 1. Draw Forward LED Headlight Beam illuminating the rails ahead of the train
        let nose_y = train_center.y + render_h / 2.0 - 24.0 * scale;
        let beam_len = 180.0 * scale;
        let start_half_w = 20.0 * scale;
        let end_half_w = 68.0 * scale;

        let v0 = Vec2::new(train_center.x - start_half_w, nose_y);
        let v1 = Vec2::new(train_center.x + start_half_w, nose_y);
        let v2 = Vec2::new(train_center.x + end_half_w, nose_y + beam_len);
        let v3 = Vec2::new(train_center.x - end_half_w, nose_y + beam_len);
        ...
        draw_circle(train_center.x - 10.0 * scale, nose_y + 6.0 * scale, 7.0 * scale, Color::from_rgba(255, 255, 220, 255));
        draw_circle(train_center.x + 10.0 * scale, nose_y + 6.0 * scale, 7.0 * scale, Color::from_rgba(255, 255, 220, 255));
        draw_circle(train_center.x, nose_y + 6.0 * scale, 18.0 * scale, Color::from_rgba(255, 245, 160, 110));

        // 2. Draw Pre-baked Train Texture Asset
        if let Some(tex) = texture {
            draw_texture_ex(
                tex,
                train_center.x - render_w / 2.0,
                train_center.y - render_h / 2.0,
                WHITE,
                DrawTextureParams {
                    dest_size: Some(Vec2::new(render_w, render_h)),
                    ..Default::default()
                },
            );
        }
