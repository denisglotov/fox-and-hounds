...
impl Screens {
    ...
    fn draw_pagination_dots(
        ...
        let dot_spacing = (16.0 * scale).min(bounds.w / (count as f32).max(1.0));
        let total_w = (count as f32 - 1.0) * dot_spacing;
        let start_x = bounds.x + (bounds.w - total_w) / 2.0;
